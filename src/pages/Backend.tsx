...
const LOGS: LogRow[] = [
  ...
  { time: '14:51:58', level: 'INFO',  source: 'socket',      msg: 'client connected · user 4231',                     req: 'ws_c8d1'  },
  { time: '14:51:41', level: 'WARN',  source: 'query',       msg: 'slow query · SELECT ... FROM reservas',  tail: '720 ms', req: 'req_a0f2' },
  { time: '14:50:22', level: 'INFO',  source: 'http',        msg: 'POST /api/reservas  201',           tail: '182 ms', req: 'req_9d3e' },
  { time: '14:49:15', level: 'ERROR', source: 'sap',         msg: 'ECONNREFUSED trycloudflare tunnel', tail: 'retry',   req: 'req_8b71' },
  { time: '14:49:11', level: 'INFO',  source: 'sap',         msg: 'tunnel probe · retrying in 2s' },
  ...
  { time: '14:45:00', level: 'INFO',  source: 'cron',        msg: 'daily aggregation started' },
  { time: '14:44:59', level: 'INFO',  source: 'cron',        msg: 'aggregation ok · 458k rows',        tail: '4.2 s' },
]
...
const ERROR_GROUPS: ErrorGroup[] = [
  { count: 24, first: '2026-08-05 09:12', last: '14:49:15', source: 'sap',     signature: 'ECONNREFUSED trycloudflare tunnel',        spark: [3, 1, 4, 2, 5, 4, 3, 2] },
...
export default function Backend() {
  ...
  return (
    <div className="flex flex-col gap-6 text-[13px]">
      ...
      {/* Deploy */}
      <Section title="deploy" subtitle="último + historial · digitalocean apps api">
        <DeployCard latest={LATEST} history={HISTORY} />
      </Section>

      {/* Resources */}
      <Section title="resources" subtitle="droplet · basic-2 · 2 vCPU · 4 GB RAM">
        <MetricRow label="cpu"  value="22" unit="%" spark={CPU_SPARK} sparkColor="#9ECE6A" hint="avg 24h  20 %" />
        <MetricRow label="ram"  value="58" unit="%" spark={RAM_SPARK} sparkColor="#7DCFFF" hint="2.3 / 4 GB" />
        <MetricRow label="disk" value="34" unit="%" spark={[32,32,33,33,34,34,34,34,34,34]} sparkColor="#BB9AF7" hint="27 / 80 GB" />
        <MetricRow label="net"  value="12" unit="MB/s" spark={[8,10,14,12,16,18,14,12,10,12]} sparkColor="#BB9AF7" hint="in/out avg" />
      </Section>
      ...
      <Section
        ...
        right={<StatusBadge status="ok" label="up · 4h 32m" />}
      >
        <div className="mt-2 px-2 flex flex-col gap-1.5">
          ...
          <div className="grid grid-cols-[110px_1fr] gap-3">
            <span className="text-fg-muted">last reconnect</span>
            <span className="text-fg-secondary">
              2026-08-07 11:02  <span className="text-fg-faint">· post reboot vps</span>
            </span>
          </div>
          <div className="grid grid-cols-[110px_1fr] gap-3">
            <span className="text-fg-muted">probes 24h</span>
            <span className="text-fg-secondary">
              288 <span className="text-fg-faint">· 0 fail · 1.4s avg</span>
            </span>
          </div>
        </div>
      </Section>
      ...
      <Section title="socket.io" subtitle="conexiones activas · ws">
        <div className="mt-2 px-2 flex items-baseline gap-6">
          <div>
            <div className="text-fg-faint uppercase tracking-wide text-[10.5px]">active</div>
            <div className="text-fg-primary tabular-nums text-[22px]">47</div>
          </div>
          <div>
            <div className="text-fg-faint uppercase tracking-wide text-[10.5px]">peak 24h</div>
      ...
      {/* Endpoints */}
      <Section title="endpoints" subtitle="latencia por ruta · p95 24h · err rate">
        <div className="mt-1">
      ...
      >
        <div className="mt-2">
          ...
          <div className="flex flex-wrap items-center gap-2 mb-2 text-[11.5px] px-2">
            <span className="text-fg-faint">filter:</span>
            ...
            <label className="text-fg-faint ml-1">from</label>
            <input
              type="date"
            ...
            />
            <label className="text-fg-faint">to</label>
            <input
              type="date"
              defaultValue="2026-08-07"
              className="bg-bg-card border border-border-subtle rounded px-2 h-6 text-fg-secondary tabular-nums outline-none focus:border-brand-500/50"
      ...
      <Section
        ...
        right={<span className="text-fg-muted text-[11px]">4 groups</span>}
      >
        <div className="mt-1">
